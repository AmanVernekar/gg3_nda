...
def get_interval(k:int)->Tuple[NDArray,float]:
    state_space,dx = np.linspace(0,1,num=k,retstep=True)
...
def get_init_distr(
    ...
    init_distr[0] = norm.cdf( (states[0] + dx/2 -x0)/ (sigma * np.sqrt(dt) ) )
    init_distr[-1]= 1.0 - norm.cdf( (states[-1]-dx/2-x0)/ (sigma * np.sqrt(dt)) )
    #get the other bins from the inverse cdf
    if states.size>2:
        for i in range(1,states.size-1):
            init_distr[i] = norm.cdf((states[i]+ dx/2- x0)/(sigma* np.sqrt(dt)))- norm.cdf((states[i]-dx/2-x0)/(sigma*np.sqrt(dt)))
...
def get_transition_mtx(
    ...
    trans_mtx[:-1, -1] = 1.0 - norm.cdf( (states[-1]-dx/2-states[:-1] - beta * dt) / (sigma * np.sqrt(dt)) )
    trans_mtx[states.size-1,states.size-1] = 1.0
    #Handle floor at x=0
    trans_mtx[:-1,0]=norm.cdf((states[0] + dx/2 - states[:-1] - beta * dt )/(sigma*np.sqrt(dt)))
    if states.size>2: #Rest of the matrix
        for i in np.arange(states.size-1):
            for j in np.arange(1, states.size - 1):
                trans_mtx[i,j] =  norm.cdf((states[j]+dx/2-states[i]- beta* dt)/(sigma* np.sqrt(dt)))- norm.cdf((states[j]-dx/2-states[i] - beta* dt)/(sigma*np.sqrt(dt)))
...
if __name__=="__main__":
    ...
    x0:float = 0.2
    s0 = round(x0*K)
    state_space, dx = get_interval(K)
    dt = 1.0/ T
    pi = get_init_distr(state_space, x0, sigma, dx, dt)
    trans_mtx = get_transition_mtx(state_space, beta, sigma, dx, dt)
